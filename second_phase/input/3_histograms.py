...
def generate_histogram_ecc(file_name1, file_name2, file_name3, file_name4):
    to_remove = 1000;
    ...
    sorted_reader4 = sorted(csv_reader4, key=operator.itemgetter(2))
    ...
    layout = go.Layout(
        ...
        autosize=True,
        plot_bgcolor='#ffffff',
        title='ECC - ' + file_name1[4:-4].replace("_", " ").capitalize() + ' / ' + file_name2[4:-4].replace("_", " ").capitalize() + ' / ' + file_name3[4:-4].replace("_", " ").capitalize() + ' / ' + file_name4[4:-4].replace("_", " ").capitalize() + ' (time / occurences)',
        xaxis=dict(title='Value of time(ms)'),
        yaxis=dict(title='Number of occurrences'),
    )
    fig = go.Figure(data=data, layout=layout)
    plot(fig,
        filename= '../results/graphs/ecc/ecc_' +  file_name1[4:-4] + '_' + file_name2[4:-4] + '_' + file_name3[4:-4] + '_' + file_name4 +'.html')
...
def generate_histogram_rsa(file_name1, file_name2, file_name3, file_name4):
    ...
    for row in sorted_reader4[:-to_remove]:
        if row[0] == 'ID' or int(row[0]) >= 100000:
            continue
        xid4.append(row[0])
        time4.append(get_time(row[3]))
    ...
    data = [trace, trace2, trace3, trace4]
    layout = go.Layout(barmode='overlay', autosize=True, plot_bgcolor='#ffffff', title='RSA - ' + file_name1[4:-4].replace("_", " ").capitalize() + ' / ' + file_name2[4:-4].replace("_", " ").capitalize() + ' / ' + file_name3[4:-4].replace("_", " ").capitalize() + ' / ' + file_name4[4:-4].replace("_", " ").capitalize() + ' (time / occurences)', xaxis=dict(title='Value of time(ms)'), yaxis=dict(title='Number of occurrences'))
    fig = go.Figure(data=data, layout=layout)
    plot(fig,
...
generate_histogram_ecc('ecc_large_exponent.txt', 'ecc_random_exponent.txt', 'ecc_short_exponent.txt', 'ecc_random_messages.txt')
