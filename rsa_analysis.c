...
void highHammingWeightRSA(br_hmac_drbg_context* ctx) {
    /*
    size_t prime_size = (sizeof(high_iq) - 1) / 2;
    unsigned char dest[prime_size];
    hexStringToByteArray(high_iq, dest, prime_size);
    return;
    */
    br_rsa_private_key pk;
    br_rsa_public_key pbk;
    pk.p = p_high;
    pk.plen = sizeof(p_high);
    pk.q = q_high;
    pk.qlen = sizeof(q_high);
    pk.iq = iq_high;
    pk.iqlen = sizeof(iq_high);
    pk.dp = dp_high;
    pk.dplen = sizeof(dp_high);
    pk.dq = dq_high;
    pk.dqlen = sizeof(dq_high);
    pk.n_bitlen = 2048;

    pbk.e = public_high;
    pbk.elen = sizeof(public_high);
    pbk.n = modulo_high;
    pbk.nlen = sizeof(modulo_high);

    unsigned char dest[256];
    size_t encrypted;
    unsigned char encMessage[] = "jina";
    size_t messageLength = sizeof(encMessage);
    if ((encrypted = br_rsa_i31_oaep_encrypt(&ctx->vtable,ctx->digest_class, NULL, 0, &pbk, dest, 256, encMessage, messageLength)) == 0) {
        printf("ERRROR at index");
    }
    int result = (br_rsa_i31_oaep_decrypt(ctx->digest_class, NULL, 0, &pk, dest, &encrypted));
    if (!result) {
        printf("ERRROR at index");
